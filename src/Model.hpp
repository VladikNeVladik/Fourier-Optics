...
void fast_fourier_transform(std::complex<double>* src, std::complex<double>* dst, std::complex<double>* roots_of_unity)
{
	...
	//---------------------------------
	//   L0           L1        L2
	//   G0 G1 G2 G3  G0   G1   G0
	//   E  E  E  E   E E  E E  E  E  E  E

	for (uint32_t level = 0; level < LOG_X; ++level)
	{
		// Level characteristics:
		uint32_t group_size = 2 << level;
		uint32_t entry_size = 1 << level;
		...
		for (uint32_t group_x = 0; group_x < SIZE_X; group_x += group_size)
		{
			...
			for (uint32_t entry_x = 0; entry_x < entry_size; ++entry_x)
			{
...
void reverse(std::complex<double>* arr)
{
	std::complex<double> tmp;
	for (uint32_t x = 0; x < SIZE_X/2; ++x)
	{
		tmp             = arr[x];
		arr[x]          = arr[SIZE_X - x];
		arr[SIZE_X - x] = tmp;
...
void apply_screen(std::complex<double>* amps, uint32_t z, uint32_t wave_z)
{
	...
	for (uint32_t x = 0; x < SIZE_X; ++x)
	{
		...
		// Account turbulence:
		amps[x] *= std::exp(std::complex<double>(0.0, 1.0) * (n - 1.0) * sqrt);
